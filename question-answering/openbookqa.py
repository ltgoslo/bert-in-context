...
def format_prompt(args, inputs: List[dict]):
    ...
    prompt_template = "{question_stem} {answer}"
    examples = [
...
def load_data(args):
    ...
    def to_dict(sample):
        ...
        d["options"] = d["choices"]["text"]
        d["options"] = [answer.strip() + '.' if not answer.endswith(".") else answer for answer in d["options"]]
        d["label"] = d["choices"]["label"].index(d["answerKey"])
...
def predict(args, model, samples, verbose=False):
    ...
    for option in samples[-1]["options"]:
        ...
        score_length = len(inputs[-1]["answer"].split()) + len(inputs[-1]["question_stem"].split())

        if verbose:
            print(input_text)
            print(score_length, flush=True)
        ...
        conditional_logp = model["model"].score(
            inputs[-1]["answer"],
            len(inputs[-1]["answer"]),
            model["tokenizer"],
            "cuda",
...
def main():
    ...
    log_file = open(
        f"result_openbookqa_{args.model_name_or_path.split('/')[-1]}_{args.n_shots}-shots.txt",
        "w",
    )
    accuracies = []
