...
def load_data(args):
    dataset = load_dataset("allenai/ai2_arc", args.subset)
...
def predict(args, model, samples, verbose=False):
    ...
    for option in samples[-1]["options"]:
        ...
        score_length = len(inputs[-1]["answer"].split()) + len(inputs[-1]["question"].split()) + 1
...
def main():
    ...
    log_file = open(
        f"result_{args.subset}_{args.model_name_or_path.split('/')[-1]}_{args.n_shots}-shots.txt",
        "w",
    )
    accuracies = []
