...
def load_data():
    ...
    dataset = load_dataset("web_questions", cache_dir='.')

    train_dataset = [
        {
            "question": sample["question"].strip(),
            "answers": [
                answer.strip() for answer in sample["answers"]
            ],
            "preferred_answer": sample["answers"][0].strip()
        }
...
def main():
    ...
    log_file = open(
        f"eval_webq_{args.model_name_or_path.split('/')[-1]}_{args.n_shots}-shots.txt",
        "w",
    )
