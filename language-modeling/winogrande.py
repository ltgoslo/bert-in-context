...
def load_data(args):
    ...
    def to_dict(sample):
        ...
        d["options"] = [d["sentence"].replace("_", option.strip()) for option in (d["option1"], d["option2"])]
        d["label"] = int(d["answer"]) - 1
...
def sample_random_examples(dataset: List[dict], n_shots: int, example_index: int):
    sequence = [i for i in range(len(dataset))]
...
def main():
    ...
    log_file = open(
        f"result_winogrande_{args.model_name_or_path.split('/')[-1]}_{args.n_shots}-shots.txt",
        "w",
    )
